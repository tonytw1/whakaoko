...
public class TwitterFeedItemMapper {
    public FeedItem createFeedItemFrom(Status status, Subscription subscription) {
        ...
        return new FeedItem(extractHeadingFrom(status), extractUrlFrom(status), null, status.getCreatedAt(), place, mediaUrl, author, subscription.getId(), subscription.getChannelId());
    }
